...
r = np.concatenate((np.zeros((1, )), ((T ** 2) * star_mass * G / (4 * (np.pi ** 2))) ** (1 / 3)))
v = np.concatenate((np.zeros((1, )), (star_mass * G / r[1:]) ** (1 / 2)))
dq0 = np.concatenate((np.zeros((n, )), v, np.zeros((n, ))))

m = np.array([star_mass] + [planets_mass for _ in range(n-1)])
mm = m[:, None] * m[None, :]
q = np.concatenate((r, np.zeros((2 * n, )))) # both y and z to 0
p = np.concatenate((dq0[:n] / m, dq0[n:2 * n] / m, dq0[2 * n:] / m))
